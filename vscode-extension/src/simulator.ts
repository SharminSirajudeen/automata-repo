...
export class AutomataSimulator {
    ...
    async simulate(document: vscode.TextDocument): Promise<void> {
        try {
    ...
    async testInput(document: vscode.TextDocument, input: string): Promise<void> {
        try {
            const text = document.getText();
            const type = this.getAutomataType(document.uri);
            const automaton = this.parser.parse(text, type);
    ...
    private async runSimulation(automaton: Automaton, input: string): Promise<{ accepted: boolean, trace: any[] }> {
        switch (automaton.type) {
            case 'dfa':
                return this.simulateDFA(automaton as DFA, input);
            case 'nfa':
                return this.simulateNFA(automaton as NFA, input);
            case 'tm':
                return this.simulateTuringMachine(automaton as TuringMachine, input);
            default:
                throw new Error(`Unknown automaton type: ${automaton.type}`);
        }
    }

    private async simulateDFA(dfa: DFA, input: string): Promise<{ accepted: boolean, trace: any[] }> {
        const trace: any[] = [];
    ...
    private async simulateNFA(nfa: NFA, input: string): Promise<{ accepted: boolean, trace: any[] }> {
        const trace: any[] = [];
    ...
    private async simulateTuringMachine(tm: TuringMachine, input: string): Promise<{ accepted: boolean, trace: any[] }> {
        const trace: any[] = [];
    ...
    private formatTape(tape: string[], position: number, windowSize: number = 10): string {
    ...
    private async showSimulationTrace(title: string, trace: any[]): Promise<void> {
        const options = ['Show in Output', 'Copy to Clipboard', 'Save to File'];
        const choice = await vscode.window.showInformationMessage(
            `${title} completed. View trace?`,
            ...options
        );
    ...
    private async showTuringMachineTrace(title: string, trace: any[]): Promise<void> {
        const options = ['Show in Output', 'Copy to Clipboard', 'Save to File'];
        const choice = await vscode.window.showInformationMessage(
            `${title} completed. View trace?`,
            ...options
    ...
    private formatTuringMachineTrace(trace: any[]): string {
    ...
    private showInOutput(title: string, content: string): void {
    ...
    private async saveTraceToFile(title: string, content: string): Promise<void> {
