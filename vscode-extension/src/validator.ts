...
import { Automaton, DFA, NFA, TuringMachine } from './parser';

export class AutomataValidator {
    validate(automaton: Automaton, document: TextDocument): Diagnostic[] {
        ...
        switch (automaton.type) {
            case 'dfa':
                this.validateDFA(automaton as DFA, document, diagnostics);
                break;
            case 'nfa':
                this.validateNFA(automaton as NFA, document, diagnostics);
                break;
            case 'tm':
                this.validateTM(automaton as TuringMachine, document, diagnostics);
                break;
        }
    ...
    private validateStructure(automaton: Automaton, document: TextDocument, diagnostics: Diagnostic[]): void {
    ...
    private validateDFA(dfa: DFA, document: TextDocument, diagnostics: Diagnostic[]): void {
    ...
    private validateNFA(nfa: NFA, document: TextDocument, diagnostics: Diagnostic[]): void {
    ...
    private validateTM(tm: TuringMachine, document: TextDocument, diagnostics: Diagnostic[]): void {
    ...
    private validateSemantics(automaton: Automaton, document: TextDocument, diagnostics: Diagnostic[]): void {
    ...
    private getReachableStates(automaton: Automaton): Set<string> {
        ...
        while (queue.length > 0) {
            ...
            if (stateTransitions) {
                for (const destinations of Object.values(stateTransitions)) {
                    switch (automaton.type) {
                        case 'dfa':
                            if (!reachable.has(destinations as string)) {
                                queue.push(destinations as string);
                            }
                            break;
                        case 'nfa':
                            ...
                            break;
                        case 'tm':
                            const tmDest = (destinations as any).state;
    ...
    private findMissingTransitions(automaton: Automaton): string[] {
        ...
        for (const state of automaton.states) {
            ...
            const alphabet = automaton.type === 'tm' ? 
                (automaton as TuringMachine).tape_alphabet : 
                automaton.alphabet;
    ...
    private findTrapStates(dfa: DFA): string[] {
        const trapStates: string[] = [];
        const acceptingStates = new Set(dfa.accept_states);
        
        for (const state of dfa.states) {
            if (acceptingStates.has(state)) continue;
    ...
    private canReachAcceptingStateFrom(automaton: Automaton, startState: string): boolean {
        ...
        while (queue.length > 0) {
            ...
            visited.add(current);
            if (acceptingStates.has(current)) return true;
                ...
                for (const destinations of Object.values(stateTransitions)) {
                    switch (automaton.type) {
                        ...
                        case 'nfa':
                            ...
                            break;
                        case 'tm':
                            queue.push((destinations as any).state);
                            break;
                    }
                }
            }
    ...
    private canReachAcceptingState(automaton: Automaton): boolean {
        return this.canReachAcceptingStateFrom(automaton, automaton.start_state);
    }

    private findDeadStates(automaton: Automaton): string[] {
    ...
    private isNFADeterministic(nfa: NFA): boolean {
    ...
    private findPotentialInfiniteLoops(tm: TuringMachine): string[] {
    ...
    private findFieldRange(document: TextDocument, field: string) {
    ...
    private findTransitionRange(document: TextDocument, state: string, symbol: string) {
    ...
    async validateDocument(document: TextDocument): Promise<Diagnostic[]> {
        try {
            ...
            let type: 'dfa' | 'nfa' | 'tm' = 'dfa';
            if (document.uri.endsWith('.nfa')) type = 'nfa';
            else if (document.uri.endsWith('.tm')) type = 'tm';
            
            const automaton = { ...json, type } as Automaton;
