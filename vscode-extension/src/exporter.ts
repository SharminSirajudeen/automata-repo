...
export class AutomataExporter {
    ...
    private async performExport(automaton: Automaton, format: string, document: vscode.TextDocument): Promise<void> {
    ...
    private generateSVG(automaton: Automaton): string {
        const width = 800;
        const height = 600;
    ...
    private generateSVGStates(automaton: Automaton, positions: Map<string, {x: number, y: number}>, radius: number): string {
        let svg = '';
    ...
    private generateSVGTransitions(automaton: Automaton, positions: Map<string, {x: number, y: number}>, stateRadius: number): string {
        let svg = '';
        ...
        for (const [fromState, stateTransitions] of Object.entries(automaton.transitions)) {
            ...
            for (const [symbol, destination] of Object.entries(stateTransitions)) {
                ...
                switch (automaton.type) {
                    case 'dfa':
                        targetState = destination as string;
                        label = symbol;
                        break;
                    case 'nfa':
    ...
    private generateSVGTransition(fromPos: {x: number, y: number}, toPos: {x: number, y: number}, radius: number, label: string): string {
    ...
    private generateSVGSelfLoop(pos: {x: number, y: number}, radius: number, label: string): string {
        const loopRadius = 25;
        const centerX = pos.x;
    ...
    private calculateStatePositions(states: string[], width: number, height: number, radius: number): Map<string, {x: number, y: number}> {
    ...
    private generateDOT(automaton: Automaton): string {
    ...
    private generateJFLAP(automaton: Automaton): string {
    ...
    private generateLaTeX(automaton: Automaton): string {
        ...
        states.forEach((state, index) => {
            ...
            let stateOptions = [];
            if (isInitial) stateOptions.push('initial');
            if (isAccepting) stateOptions.push('accepting');
    ...
    private async exportToPNG(automaton: Automaton, baseName: string): Promise<void> {
    ...
    private async exportToPDF(automaton: Automaton, baseName: string): Promise<void> {
    ...
    private generatePDFReport(automaton: Automaton): string {
            ...
            for (const [symbol, destination] of Object.entries(stateTransitions)) {
                switch (automaton.type) {
                    case 'dfa':
                        ...
                        break;
                    case 'nfa':
                        const destinations = (destination as string[]).join(', ');
    ...
    private async saveToFile(content: string, defaultFileName: string, extension: string): Promise<void> {
        ...
        if (uri) {
            ...
            const openChoice = await vscode.window.showInformationMessage(
                'Export completed. Open the file?',
                'Open', 'Open Folder'
            );
    ...
    private getFileFilters(extension: string): { [key: string]: string[] } {
