...
class AutomataPreviewProvider {
    ...
    generateHtml(automaton, fileName) {
        const scriptUri = AutomataPreviewProvider.currentPanel.webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'preview.js'));
                        ...
                        <div class="automata-details">
                            ...
                            ${automaton.type === 'tm' ? `
                                <div class="detail-section">
                                    <h4>Tape Alphabet</h4>
                                    <div class="alphabet-list">
                                        ${automaton.tape_alphabet.map((symbol) => `<span class="symbol-item ${symbol === automaton.blank_symbol ? 'blank' : ''}">${symbol}</span>`).join('')}
                                    </div>
                                </div>
                            ` : ''}
    ...
    generateTransitionList(automaton) {
        const transitions = [];
            ...
            for (const [symbol, destination] of Object.entries(stateTransitions)) {
                switch (automaton.type) {
                    ...
                    case 'nfa':
                        ...
                        break;
                    case 'tm':
                        const tmTransition = destination;
